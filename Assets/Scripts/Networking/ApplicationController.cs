...
public class ApplicationController : MonoBehaviour
{
    ...
    private async Task LaunchInMode(bool isDedicatedServer)
    {
        if (isDedicatedServer)
        {
            ...
            StartCoroutine(LoadGameSceneAsync(ServerSingleton.Instance));
        }
        else
        {
            Host.Instance.Create(playerPrefab);

            bool authenticated = await Client.Instance.Create();

            if (authenticated)
                Client.Instance.GameManager.GoToMenu();
        }
    }
